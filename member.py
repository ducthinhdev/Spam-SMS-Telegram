...
bot_token = '6396892383:AAEQaSL9tqTVOpQeOB4tRXmolQODdjgL3DU' 
bot = telebot.TeleBot(bot_token)

allowed_group_id = -908024550
...
def laykey(message):
    ...
    string = f'Jiray-{username}+{TimeStamp()}'
    hash_object = hashlib.md5(string.encode())
    key = str(hash_object.hexdigest())
    print(key)
    url_key = requests.get(f'https://web1s.com/api?token=75cbf980-f417-4f1d-8d6b-b6f09ae86c63&url=https://keytool.thinhcuteno1.repl.co/?key={key}').json()['shortenedUrl']
...
def key(message):
    ...
    if key == expected_key or key=="thinhdz":
        allowed_users.append(user_id)
...
def lqm_sms(message):
    ...
    if user_id not in allowed_users:
        bot.reply_to(message, text='🚀VUI LÒNG NHẬP KEY ĐỂ SỬ DỤNG TOOL BẰNG CÁCH : /KEY {KEY}🚀')
        return
    if len(message.text.split()) == 1:
    ...
    if phone_number in ['113','911','114','115','+84978200223','0978200223','0915992123']:
